...
/*   Updated: 2023/05/03 15:25:27 by lgirault         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	free_all2(char **res, int j)
{
	int	i;
...
static int	count_line2(t_var var)
{
	int	count;
...
static char	**split_incurve4(char **res, t_var var,
			t_ms *ms, t_cmd_lst *cmd_lst)
{
	while (var.newstr[var.i] != '\0')
	{
		...
		while (var.newstr[var.i] <= var.c
			...
			var.i++;
		res[var.j] = malloc(sizeof(char) * (count_line2(var) + 1));
		if (!res[var.j])
		{
			free_all2(res, var.j);
...
char	**split_incurv3(char *str, char c, t_ms *ms, t_cmd_lst *cmd_lst)
{
	...
	var.c = c;
	var.newstr = str;
	res = malloc(sizeof(char *) * (count_words2(var.newstr, var.c) + 1));
	if (!res)
		free_parsing(ms, cmd_lst, var.newstr);
	res = split_incurve4(res, var, ms, cmd_lst);
	return (res);
}
