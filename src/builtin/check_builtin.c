...
void	only_last(t_cmd_lst *cmd_lst, t_ms *ms)
{
	...
	if (cmd_lst->next == NULL && ft_strcmp(cmd_lst->cmd_option[0], "export") == 0 && cmd_lst->cmd_option[1] != NULL && cmd_lst->cmd_option[1][0] != '-')//segfault si export tout seul
		export_builtin(cmd_lst, ms);
	if (cmd_lst->next == NULL && ft_strcmp(cmd_lst->cmd_option[0], "unset") == SUC && cmd_lst->cmd_option[1][0] != '-')
