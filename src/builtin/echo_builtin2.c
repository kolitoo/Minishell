...
/*   Updated: 2023/05/05 20:18:47 by lgirault         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int    len_last_check(char *str)
{
    ...
    while (str[++i] != '\0')
    {
        if (((str[i] == '$' && str[i + 1] == '\'') || (str[i] == '$' && str[i + 1] == '\"')) && bool_cote(str, i) == ERR)
            i++;
        len++;
    }
    return (len);
...
char    *dollar_last_check(char *str,t_ms **ms, t_cmd_lst *cmd_lst)
{
    ...
    while (str[++var.i] != '\0')
    {
        if (((str[var.i] == '$' && str[var.i + 1] == '\'') || (str[var.i] == '$' && str[var.i + 1] == '\"')) && bool_cote(str, var.i) == ERR)
            var.i++;
